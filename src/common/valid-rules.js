...
const validate = {
  ...
  checkSubnetMask(dnsValue, [ipAddr]) {
    ...
    let ipAddressArr = ipAddr.split(".");
    let dnsAddressArr = dnsValue.split(".");
    let isNumber = false;
    ...
    if (isNumber) {
      if (ipAddressArr.length === 4) {
        ...
        } else if (ipAddressArr[0] === "172") {
          if (dnsValue === "255.255.255.0" || dnsValue === "255.255.0.0") {
            return true;
          } else {
            return false;
          }
        } else if (ipAddressArr[0] === "10") {
          if (
  ...
  checkGateway(value, [gatewayAddr, ipAddr]) {
    ...
    let ipAddressArr = ipAddr.split(".");
    let gatewayAddressArr = gatewayAddr.split(".");
    ...
    if (gatewayAddressArr.length === 4) {
      ...
      if (ipAddressArr[3] === gatewayAddressArr[3]) {
        ...
        if (gatewayAddressArr[3] === "0" || gatewayAddressArr[3] !== "1") {
          fourthOctet = false;
        } else {
          fourthOctet = true;
        }
  ...
  checkValidStartAndendIP(_, [startIP, endIP]) {
    let listOctetStart = startIP.split(".");
    let listOctetEnd = endIP.split(".");
    for (let i = 0; i < listOctetStart.length; i++) {
      let startNumber = parseInt(listOctetStart[i]),
        endNumber = parseInt(listOctetEnd[i]);
  ...
  checkRangeIP(_, [startIP, endIP]) {
    ...
    isClassDiff = classRangeEnd > classRangeStart ? true : false;
    if (isClassDiff) {
      return true;
    } else {
  ...
  checkDuplicateRouteEntry(_, [id, destinationIP, listRouting]) {
    for (let i = 0; i < listRouting.length; i++) {
      if (id !== null && id !== undefined) {
        ...
        if (destinationIP === listRouting[i].destinationIP) {
          return false;
        }
      }
    }
  ...
  existedIpFilteringRule(_, [value, list, index]) {
    ...
    if (vListRule.length === 0) {
      ...
      for (let i = 0; i < vListRule.length; i++) {
        ...
        if (
          compareObjects(vListRule[i].src, value.src) &&
          compareObjects(vListRule[i].dest, value.dest) &&
